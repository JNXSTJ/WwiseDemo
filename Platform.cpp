...
AkOSChar* GetExecutablePath()
{
	static AkOSChar executablePath[MAX_PATH];
	...
	if (result > 0)
	{
		...
		AKPLATFORM::SafeStrCpy(executablePath, path, MAX_PATH);
	}
...
AkOSChar* GetSoundBanksPath()
{
	...
	if (pathLen == 0)
	{
		AkOSChar *path = GetExecutablePath();
...
{
	static AkOSChar pluginDllPath[MAX_PATH];
	static size_t pathLen = 0;
	...
	if (pathLen == 0)
	{
		...
		// Copy the path to its final destination
		AKPLATFORM::SafeStrCpy(pluginDllPath, path, MAX_PATH);
		// Concatenate the soundbanks path
		AKPLATFORM::SafeStrCat(pluginDllPath, DLL_PATH, MAX_PATH);
		// Shorten the path as much as possible by removing the "\.."
		ShortenPath(pluginDllPath);
		pathLen = AKPLATFORM::OsStrLen(pluginDllPath);
	}
